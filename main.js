(async () => {
  ...
  if (isPairing && !conn.authState.creds.registered) {
    CFonts.say("WELCOME", {
      ...
      CFonts.say("Powered By Fuzan", {
        colors: ["system"],
        font: "console",
        align: "center",
      });
    console.log(
      chalk.white.bold(`Operating System Information:
...
- Uptime: ${os.uptime()} seconds`)
    ...
    console.log(
      `${chalk.green.bold("- Your Paring Code : ")} ${chalk.yellow.bold(
        code
      )}\n\n`
    );
  }
  async function connectionUpdate(update) {
    ...
    if (connection == "open") {
      console.log(
        chalk.yellow.bold(
          `[ Success ] Success connect to : ${JSON.stringify(
            conn.user,
            null,
            2
          )}`
  ...
  try {
    ...
    watcher
      ...
      .on("unlink", (filename) => {
        console.log(
          chalk.yellow.bold(
            "[ Delete ] Suucess Delete : " + filename.replace(process.cwd(), "")
          )
        );
