        ...
        ...Object.assign({
          contextInfo: {
            ...
            forwardedNewsletterMessageInfo: {
              ...
              serverMessageId: 173,
              newsletterName: `Runtime: ${runtime(
                process.uptime()
              )} 🚀 || ${textsaluran}`,
            },
          },
      ...
      let mediaMessage = await prepareWAMessageMedia(
        {
          ...(mime === "image"
            ...
            : {
                document: file.data,
                mimetype: file.mime,
                fileName: json.filename || "Nazir" + extension(file.mime),
              }),
        },
        { upload: conn.waUploadToServer }
      );
...
exports.smsg = (conn, m, hasParent) => {
  ...
  m.reply = async (pesan, options) => {
    try {
      if (options && pesan) {
        conn.sendFile(m.chat, options, null, pesan, m);
      } else {
        if (pesan) {
          conn.reply(m.chat, pesan, m);
        } else {
          conn.reply(m.chat, options, m);
        }
      }
    } catch (e) {
      conn.reply(m.chat, pesan, e);
    }
  };
