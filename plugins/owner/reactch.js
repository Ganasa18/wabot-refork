...
let handler = async (m, { conn, text }) => {
  if (!text)
    return m.reply(
      "Contoh penggunaan: .rch https://whatsapp.com/channel/xxx Hai"
    );

  let args = text.split(" ");
  if (args.length < 2)
    return m.reply(
      "Format salah! Contoh: .rch https://whatsapp.com/channel/xxx Hai"
    );
  ...
  try {
    ...
    m.reply(
      `Reaksi berhasil dikirim ke ${res.name} dengan kode ${kode} dan emoji: ${emoji}`
    );
  } catch (error) {
