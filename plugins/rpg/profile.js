...
let handler = async (m, { conn, command }) => {
  try {
    ...
    let ppUrl = await conn
      ...
      .catch(
        (_) =>
          "https://cdn.jsdelivr.net/gh/SazumiVicky/MakeMeow-Storage@main/avatar_contact.png"
      );
    ...
    let pasangan = user.pasangan
      ? global.db.data.users[user.pasangan].name
      : "Not Have"; // Mengambil nama pasangan dari database
    let banned = user.banned ? true : false;
    let sahabat = user.sahabat
      ? "" + global.db.data.users[user.sahabat].name
      : "Not Have";
