...
let handler = async (m, { conn, usedPrefix, command, text }) => {
  if (!text.includes(","))
    return m.reply(
      "Format salah! Gunakan: jarak [kota asal],[kota tujuan]\nContoh: jarak bekasi,madiun"
    );
  ...
  let biyumaunyepong = `https://api.vreden.my.id/api/tools/jarak?from=${encodeURIComponent(
    from
  ...
  try {
    ...
    if (data.status !== 200)
      return m.reply(
        "Gagal mendapatkan data jarak! Pastikan kota yang dimasukkan benar."
      );
...
⛽ *Estimasi BBM:* ${result.estimasi_biaya_bbm.total_liter} liter (~${
      result.estimasi_biaya_bbm.total_biaya
    })

🗺️ *Peta:* ${result.peta_statis}

📍 *Rute Perjalanan:* 
${result.arah_penunjuk_jalan
  .map((step) => `🚘 ${step.instruksi} (${step.jarak})`)
  .join("\n")}`;
    m.reply(msg);
  } catch (error) {
    console.error(error);
